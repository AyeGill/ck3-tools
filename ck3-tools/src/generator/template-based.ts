...
import { CK3Formatter } from '../common/formatter';
...
export class TemplateGenerator {
  ...
  ): { content: string; localization: string } {
    ...
    for (const [key, value] of Object.entries(structure)) {
      ...
      if (key === 'can_construct_potential' || key === 'can_construct_showing_failures_only') {
        ...
        if (value && typeof value === 'object' && 'condition' in value) {
          formatter.writeLine((value as any).condition);
        } else {
        ...
        if (value && typeof value === 'object') {
          if ('base' in value) {
            formatter.writeProperty('base', (value as any).base);
            formatter.writeLine();
          }
          if ('modifiers' in value) {
            for (const mod of (value as any).modifiers) {
        ...
        for (const [modKey, modValue] of Object.entries(value)) {
          if (modKey !== 'condition') {
            formatter.writeProperty(modKey, modValue as any);
          }
        }
        formatter.endBlock();
        formatter.writeLine();
          ...
          if (typeof item === 'object' && item !== null) {
            ...
            for (const [itemKey, itemValue] of Object.entries(item)) {
              if (typeof itemValue === 'object' && itemValue !== null && !Array.isArray(itemValue)) {
                ...
                formatter.writeProperty(itemKey, itemValue as any);
              }
            }
          } else {
        ...
        formatter.writeProperty(key, value as any);
        formatter.writeLine();
      }
    }
  }
  ...
  private generateTrait(
    ...
    if (levels && levels > 1) {
      ...
      // Single trait
      const context = { ...params, formatted_name: params.name.split('_').map((w: string) => w.charAt(0).toUpperCase() + w.slice(1)).join(' ') };
      const structure = this.engine.processObject(template.structure, context);
   ...
   */
  private generateTraitLocalization(
    template: TemplateDefinition,
    params: Record<string, any>
  ): string {
    const { name, levels } = params;
   ...
   */
  private writeDecisionStructure(formatter: CK3Formatter, structure: any): void {
    for (const [key, value] of Object.entries(structure)) {
      if (key === 'is_shown' || key === 'is_valid' || key === 'is_valid_showing_failures_only') {
        ...
        if (typeof value === 'string') {
          ...
          for (const line of lines) {
            if (line.trim()) formatter.writeLine(line.trim());
          }
          ...
          if ('conditions' in value) {
            ...
            if (typeof value.conditions === 'string') {
              ...
              for (const condition of Object.values(value.conditions)) {
                if (condition) formatter.writeLine(condition as string);
              }
            }
          } else {
        ...
        if (value && typeof value === 'object') {
          for (const [k, v] of Object.entries(value)) {
            if (v !== undefined && v !== null && v !== '') {
              formatter.writeProperty(k, v as any);
            }
          }
        }
        formatter.endBlock();
        ...
        if (value && typeof value === 'object') {
          ...
          if ('modifiers' in value) {
            for (const mod of (value as any).modifiers) {
              formatter.startBlock('modifier');
              if (mod.add !== undefined) formatter.writeProperty('add', mod.add);
              if (mod.factor !== undefined) formatter.writeProperty('factor', mod.factor);
              if (mod.condition) formatter.writeLine(mod.condition);
              formatter.endBlock();
              formatter.writeLine();
            }
          }
   ...
   */
  private generateDecisionLocalization(
    template: TemplateDefinition,
    params: Record<string, any>
  ): string {
    const { name } = params;
   ...
   */
  private writeEventStructure(formatter: CK3Formatter, structure: any): void {
    for (const [key, value] of Object.entries(structure)) {
      // Skip keys that are part of namespace declaration or id
      if (key === 'event_type' || key === 'namespace' || key === 'id') {
        if (key === 'event_type') {
          formatter.writeProperty('type', value as any);
          formatter.writeLine();
        }
        continue;
      }
      ...
      if (key === 'title' || key === 'desc') {
        ...
        formatter.writeProperty('theme', value as any);
        formatter.writeLine();
        ...
        if (value && typeof value === 'string' && value.trim()) {
          ...
          formatter.writeLine(value as string);
          formatter.endBlock();
          formatter.writeLine();
        }
      } else if (key === 'option') {
   ...
   */
  private writeEventOption(formatter: CK3Formatter, option: any): void {
    for (const [key, value] of Object.entries(option)) {
      if (key === 'name') {
        formatter.writeProperty('name', value as any);
        formatter.writeLine();
   ...
   */
  private generateEventLocalization(
    template: TemplateDefinition,
    params: Record<string, any>
  ): string {
