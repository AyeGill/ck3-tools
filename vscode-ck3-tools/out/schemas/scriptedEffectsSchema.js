...
exports.scriptedEffectSchemaMap = exports.scriptedEffectSchema = void 0;
...
exports.scriptedEffectSchemaMap = new Map(exports.scriptedEffectSchema.filter(f => !f.isWildcard).map((field) => [field.name, field]));
// Get all field names for completion (non-wildcard only, effects come from data)
function getScriptedEffectFieldNames() {
    return exports.scriptedEffectSchema.filter(f => !f.isWildcard).map((field) => field.name);
}
