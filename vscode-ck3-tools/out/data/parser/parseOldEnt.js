 ...
 */
function extractParameters(syntax, name) {
    if (!syntax)
        return [];
...
function parseEffects(content) {
    ...
    for (const entry of entries) {
        ...
        if (!trimmed || trimmed === 'Effect Documentation:')
            continue;
...
function parseTriggers(content) {
    ...
    for (const entry of entries) {
        const trimmed = entry.trim();
        if (!trimmed || trimmed === 'Trigger Documentation:')
            continue;
        const lines = trimmed.split('\n').map(l => l.trim()).filter(l => l);
        if (lines.length === 0)
            continue;
 ...
 */
function generateEffectsCode(effects) {
    const lines = [
        '/**',
 ...
 */
function generateTriggersCode(triggers) {
    const lines = [
        '/**',
...
main();
//# sourceMappingURL=parseOldEnt.js.map