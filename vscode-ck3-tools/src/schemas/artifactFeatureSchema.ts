...
export const artifactFeatureSchemaMap = new Map<string, FieldSchema>(
  artifactFeatureSchema.map((field) => [field.name, field])
);

export function getArtifactFeatureFieldNames(): string[] {
  return artifactFeatureSchema.map((field) => field.name);
}

export function getArtifactFeatureFieldDocumentation(fieldName: string): string | undefined {
  const field = artifactFeatureSchemaMap.get(fieldName);
