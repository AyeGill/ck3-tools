...
export const scriptedSpawnSchema: FieldSchema[] = [
  // Basic Properties
  {
    name: 'save_scope_as',
    type: 'string',
...
export const scriptedSpawnSchemaMap = new Map<string, FieldSchema>(
  scriptedSpawnSchema.map((field) => [field.name, field])
);

export function getScriptedSpawnFieldNames(): string[] {
  return scriptedSpawnSchema.map((field) => field.name);
}

export function getScriptedSpawnFieldDocumentation(fieldName: string): string | undefined {
  const field = scriptedSpawnSchemaMap.get(fieldName);
