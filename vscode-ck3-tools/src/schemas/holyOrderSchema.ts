...
export const holyOrderSchema: FieldSchema[] = [
  // Basic Properties
  {
    name: 'name',
    type: 'string',
...
export const holyOrderSchemaMap = new Map<string, FieldSchema>(
  holyOrderSchema.map((field) => [field.name, field])
);

export function getHolyOrderFieldNames(): string[] {
  return holyOrderSchema.map((field) => field.name);
}

export function getHolyOrderFieldDocumentation(fieldName: string): string | undefined {
  const field = holyOrderSchemaMap.get(fieldName);
