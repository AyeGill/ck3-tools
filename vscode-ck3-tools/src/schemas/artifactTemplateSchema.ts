...
export function getArtifactTemplateFieldNames(): string[] {
  return artifactTemplateSchema.map((field) => field.name);
}

export function getArtifactTemplateFieldDocumentation(fieldName: string): string | undefined {
  const field = artifactTemplateSchemaMap.get(fieldName);
