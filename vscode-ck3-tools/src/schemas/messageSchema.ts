...
export const messageSchemaMap = new Map<string, FieldSchema>(
  messageSchema.map((field) => [field.name, field])
);

export function getMessageFieldNames(): string[] {
  return messageSchema.map((field) => field.name);
}

export function getMessageFieldDocumentation(fieldName: string): string | undefined {
  const field = messageSchemaMap.get(fieldName);
