...
export const pilgrimageTypeSchema: FieldSchema[] = [
  // Basic Properties
  {
    name: 'name',
    type: 'string',
...
export const pilgrimageTypeSchemaMap = new Map<string, FieldSchema>(
  pilgrimageTypeSchema.map((field) => [field.name, field])
);

export function getPilgrimageTypeFieldNames(): string[] {
  return pilgrimageTypeSchema.map((field) => field.name);
}

export function getPilgrimageTypeFieldDocumentation(fieldName: string): string | undefined {
  const field = pilgrimageTypeSchemaMap.get(fieldName);
