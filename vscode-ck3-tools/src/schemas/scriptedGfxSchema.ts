...
export const scriptedGfxSchema: FieldSchema[] = [
  // Basic Properties
  {
    name: 'name',
    type: 'string',
...
export const scriptedGfxSchemaMap = new Map<string, FieldSchema>(
  scriptedGfxSchema.map((field) => [field.name, field])
);

export function getScriptedGfxFieldNames(): string[] {
  return scriptedGfxSchema.map((field) => field.name);
}

export function getScriptedGfxFieldDocumentation(fieldName: string): string | undefined {
  const field = scriptedGfxSchemaMap.get(fieldName);
