...
export const faithSchema: FieldSchema[] = [
  // Basic Properties
  {
    name: 'color',
    type: 'block',
...
export const religionSchema: FieldSchema[] = [
  {
    name: 'family',
    type: 'enum',
    description: 'The religion family this religion belongs to.',
    values: [...RELIGION_FAMILIES],
...
export const faithSchemaMap = new Map<string, FieldSchema>(
  faithSchema.map((field) => [field.name, field])
);
...
export const religionSchemaMap = new Map<string, FieldSchema>(
  religionSchema.map((field) => [field.name, field])
);

// Get all field names for completion
export function getFaithFieldNames(): string[] {
  return faithSchema.map((field) => field.name);
}
...
export function getReligionFieldNames(): string[] {
  return religionSchema.map((field) => field.name);
}

// Get documentation for a field
export function getFaithFieldDocumentation(fieldName: string): string | undefined {
  const field = faithSchemaMap.get(fieldName);
