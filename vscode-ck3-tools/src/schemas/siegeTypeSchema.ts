...
export const siegeTypeSchema: FieldSchema[] = [
  // Basic Properties
  {
    name: 'name',
    type: 'string',
...
export const siegeTypeSchemaMap = new Map<string, FieldSchema>(
  siegeTypeSchema.map((field) => [field.name, field])
);

export function getSiegeTypeFieldNames(): string[] {
  return siegeTypeSchema.map((field) => field.name);
}

export function getSiegeTypeFieldDocumentation(fieldName: string): string | undefined {
  const field = siegeTypeSchemaMap.get(fieldName);
