...
export const titleNamingSchema: FieldSchema[] = [
  // Basic Properties
  {
    name: 'name',
    type: 'string',
...
export const titleNamingSchemaMap = new Map<string, FieldSchema>(
  titleNamingSchema.map((field) => [field.name, field])
);

export function getTitleNamingFieldNames(): string[] {
  return titleNamingSchema.map((field) => field.name);
}

export function getTitleNamingFieldDocumentation(fieldName: string): string | undefined {
  const field = titleNamingSchemaMap.get(fieldName);
