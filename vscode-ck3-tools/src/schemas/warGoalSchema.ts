...
export const warGoalSchema: FieldSchema[] = [
  // Basic Properties
  {
    name: 'war_goal_type',
    type: 'enum',
...
export const warGoalSchemaMap = new Map<string, FieldSchema>(
  warGoalSchema.map((field) => [field.name, field])
);

export function getWarGoalFieldNames(): string[] {
  return warGoalSchema.map((field) => field.name);
}

export function getWarGoalFieldDocumentation(fieldName: string): string | undefined {
  const field = warGoalSchemaMap.get(fieldName);
