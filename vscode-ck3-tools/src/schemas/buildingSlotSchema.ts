...
export const buildingSlotSchema: FieldSchema[] = [
  // Basic Properties
  {
    name: 'name',
    type: 'string',
...
export const buildingSlotSchemaMap = new Map<string, FieldSchema>(
  buildingSlotSchema.map((field) => [field.name, field])
);

export function getBuildingSlotFieldNames(): string[] {
  return buildingSlotSchema.map((field) => field.name);
}

export function getBuildingSlotFieldDocumentation(fieldName: string): string | undefined {
  const field = buildingSlotSchemaMap.get(fieldName);
