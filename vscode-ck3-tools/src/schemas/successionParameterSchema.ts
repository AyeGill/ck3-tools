...
export const successionParameterSchema: FieldSchema[] = [
  // Basic Properties
  {
    name: 'name',
    type: 'string',
...
export const successionParameterSchemaMap = new Map<string, FieldSchema>(
  successionParameterSchema.map((field) => [field.name, field])
);

export function getSuccessionParameterFieldNames(): string[] {
  return successionParameterSchema.map((field) => field.name);
}

export function getSuccessionParameterFieldDocumentation(fieldName: string): string | undefined {
  const field = successionParameterSchemaMap.get(fieldName);
