...
export const coaEmblemSchema: FieldSchema[] = [
  // Basic Properties
  {
    name: 'name',
    type: 'string',
...
export const coaEmblemSchemaMap = new Map<string, FieldSchema>(
  coaEmblemSchema.map((field) => [field.name, field])
);

export function getCoaEmblemFieldNames(): string[] {
  return coaEmblemSchema.map((field) => field.name);
}

export function getCoaEmblemFieldDocumentation(fieldName: string): string | undefined {
  const field = coaEmblemSchemaMap.get(fieldName);
