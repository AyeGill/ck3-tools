...
export const characterHistorySchema: FieldSchema[] = [
  // Basic Properties
  {
    name: 'name',
    type: 'string',
...
export const characterHistorySchemaMap = new Map<string, FieldSchema>(
  characterHistorySchema.map((field) => [field.name, field])
);

export function getCharacterHistoryFieldNames(): string[] {
  return characterHistorySchema.map((field) => field.name);
}

export function getCharacterHistoryFieldDocumentation(fieldName: string): string | undefined {
  const field = characterHistorySchemaMap.get(fieldName);
