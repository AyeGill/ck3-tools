...
export const struggleSchema: FieldSchema[] = [
  // Basic Properties
  {
    name: 'start_phase',
    type: 'string',
...
export const struggleSchemaMap = new Map<string, FieldSchema>(
  struggleSchema.map((field) => [field.name, field])
);

export function getStruggleFieldNames(): string[] {
  return struggleSchema.map((field) => field.name);
}

export function getStruggleFieldDocumentation(fieldName: string): string | undefined {
  const field = struggleSchemaMap.get(fieldName);
