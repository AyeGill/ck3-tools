...
export const courtEventSchema: FieldSchema[] = [
  // Basic Properties
  {
    name: 'name',
    type: 'string',
...
export const courtEventSchemaMap = new Map<string, FieldSchema>(
  courtEventSchema.map((field) => [field.name, field])
);

export function getCourtEventFieldNames(): string[] {
  return courtEventSchema.map((field) => field.name);
}

export function getCourtEventFieldDocumentation(fieldName: string): string | undefined {
  const field = courtEventSchemaMap.get(fieldName);
