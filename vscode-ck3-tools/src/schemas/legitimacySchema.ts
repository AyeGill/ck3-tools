...
export const legitimacySchema: FieldSchema[] = [
  // Basic Properties
  {
...
export const legitimacySchemaMap = new Map<string, FieldSchema>(
  legitimacySchema.map((field) => [field.name, field])
);

export function getLegitimacyFieldNames(): string[] {
  return legitimacySchema.map((field) => field.name);
}

export function getLegitimacyFieldDocumentation(fieldName: string): string | undefined {
  const field = legitimacySchemaMap.get(fieldName);
