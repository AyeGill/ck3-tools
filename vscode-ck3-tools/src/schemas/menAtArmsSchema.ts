...
export const menAtArmsSchema: FieldSchema[] = [
  // Basic Properties
  {
    name: 'type',
    type: 'enum',
...
export const menAtArmsSchemaMap = new Map<string, FieldSchema>(
  menAtArmsSchema.map((field) => [field.name, field])
);

// Get all field names for completion
export function getMenAtArmsFieldNames(): string[] {
  return menAtArmsSchema.map((field) => field.name);
}

// Get documentation for a field
export function getMenAtArmsFieldDocumentation(fieldName: string): string | undefined {
  const field = menAtArmsSchemaMap.get(fieldName);
