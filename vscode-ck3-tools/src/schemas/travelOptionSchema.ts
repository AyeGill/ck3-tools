...
export const travelOptionSchema: FieldSchema[] = [
  // Basic Properties
  {
    name: 'name',
    type: 'string',
...
export const travelOptionSchemaMap = new Map<string, FieldSchema>(
  travelOptionSchema.map((field) => [field.name, field])
);

export function getTravelOptionFieldNames(): string[] {
  return travelOptionSchema.map((field) => field.name);
}

export function getTravelOptionFieldDocumentation(fieldName: string): string | undefined {
  const field = travelOptionSchemaMap.get(fieldName);
