...
export const combatPhaseSchema: FieldSchema[] = [
  // Basic Properties
  {
    name: 'name',
    type: 'string',
...
export const combatPhaseSchemaMap = new Map<string, FieldSchema>(
  combatPhaseSchema.map((field) => [field.name, field])
);

export function getCombatPhaseFieldNames(): string[] {
  return combatPhaseSchema.map((field) => field.name);
}

export function getCombatPhaseFieldDocumentation(fieldName: string): string | undefined {
  const field = combatPhaseSchemaMap.get(fieldName);
