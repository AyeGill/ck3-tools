...
export const menAtArmsTypeSchema: FieldSchema[] = [
  // Basic Properties
  {
    name: 'type',
    type: 'enum',
...
export const menAtArmsTypeSchemaMap = new Map<string, FieldSchema>(
  menAtArmsTypeSchema.map((field) => [field.name, field])
);

export function getMenAtArmsTypeFieldNames(): string[] {
  return menAtArmsTypeSchema.map((field) => field.name);
}

export function getMenAtArmsTypeFieldDocumentation(fieldName: string): string | undefined {
  const field = menAtArmsTypeSchemaMap.get(fieldName);
