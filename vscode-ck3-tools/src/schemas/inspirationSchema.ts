...
export const inspirationSchema: FieldSchema[] = [
  // Basic Properties
  {
    name: 'type',
    type: 'enum',
    description: 'The type of artifact this inspiration creates.',
    values: [...INSPIRATION_TYPES],
    example: 'type = weapon',
  },
  {
    name: 'gold_cost',
...
export const inspirationSchemaMap = new Map<string, FieldSchema>(
  inspirationSchema.map((field) => [field.name, field])
);

export function getInspirationFieldNames(): string[] {
  return inspirationSchema.map((field) => field.name);
}

export function getInspirationFieldDocumentation(fieldName: string): string | undefined {
  const field = inspirationSchemaMap.get(fieldName);
