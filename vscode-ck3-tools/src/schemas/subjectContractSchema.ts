...
export const subjectContractSchema: FieldSchema[] = [
  // Basic Properties
  {
    name: 'contract_type',
    type: 'string',
...
export const subjectContractSchemaMap = new Map<string, FieldSchema>(
  subjectContractSchema.map((field) => [field.name, field])
);

export function getSubjectContractFieldNames(): string[] {
  return subjectContractSchema.map((field) => field.name);
}

export function getSubjectContractFieldDocumentation(fieldName: string): string | undefined {
  const field = subjectContractSchemaMap.get(fieldName);
