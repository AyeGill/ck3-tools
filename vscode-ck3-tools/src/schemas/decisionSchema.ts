...
export const decisionSchema: FieldSchema[] = [
  // Basic Properties
  {
    name: 'desc',
    type: 'string',
...
export const decisionSchemaMap = new Map<string, FieldSchema>(
  decisionSchema.map((field) => [field.name, field])
);

/**
 * Schema for decision picture blocks
 */
export const decisionPictureSchema: FieldSchema[] = [
  {
    name: 'reference',
    type: 'string',
...
export const decisionPictureSchemaMap = new Map<string, FieldSchema>(
  decisionPictureSchema.map((field) => [field.name, field])
);

/**
 * Schema for cost blocks
 */
export const costBlockSchema: FieldSchema[] = [
  {
    name: 'gold',
    type: 'integer',
...
export const costBlockSchemaMap = new Map<string, FieldSchema>(
  costBlockSchema.map((field) => [field.name, field])
);

// Get all field names for completion
export function getDecisionFieldNames(): string[] {
  return decisionSchema.map((field) => field.name);
}

// Get documentation for a field
export function getDecisionFieldDocumentation(fieldName: string): string | undefined {
  const field = decisionSchemaMap.get(fieldName);
