...
export const warContributionSchema: FieldSchema[] = [
  // Basic Properties
  {
    name: 'name',
    type: 'string',
...
export const warContributionSchemaMap = new Map<string, FieldSchema>(
  warContributionSchema.map((field) => [field.name, field])
);

export function getWarContributionFieldNames(): string[] {
  return warContributionSchema.map((field) => field.name);
}

export function getWarContributionFieldDocumentation(fieldName: string): string | undefined {
  const field = warContributionSchemaMap.get(fieldName);
