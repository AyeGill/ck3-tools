...
export const executionMethodSchema: FieldSchema[] = [
  // Basic Properties
  {
    name: 'name',
    type: 'string',
...
export const executionMethodSchemaMap = new Map<string, FieldSchema>(
  executionMethodSchema.map((field) => [field.name, field])
);

export function getExecutionMethodFieldNames(): string[] {
  return executionMethodSchema.map((field) => field.name);
}

export function getExecutionMethodFieldDocumentation(fieldName: string): string | undefined {
  const field = executionMethodSchemaMap.get(fieldName);
