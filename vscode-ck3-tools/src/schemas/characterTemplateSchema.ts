...
export const characterTemplateSchema: FieldSchema[] = [
  // Basic Properties
  {
    name: 'name',
    type: 'string',
...
export const characterTemplateSchemaMap = new Map<string, FieldSchema>(
  characterTemplateSchema.map((field) => [field.name, field])
);

export function getCharacterTemplateFieldNames(): string[] {
  return characterTemplateSchema.map((field) => field.name);
}

export function getCharacterTemplateFieldDocumentation(fieldName: string): string | undefined {
  const field = characterTemplateSchemaMap.get(fieldName);
