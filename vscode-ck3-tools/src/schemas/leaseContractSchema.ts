...
export const leaseContractSchema: FieldSchema[] = [
  // Basic Properties
  {
    name: 'lease_type',
    type: 'enum',
...
export const leaseContractSchemaMap = new Map<string, FieldSchema>(
  leaseContractSchema.map((field) => [field.name, field])
);

export function getLeaseContractFieldNames(): string[] {
  return leaseContractSchema.map((field) => field.name);
}

export function getLeaseContractFieldDocumentation(fieldName: string): string | undefined {
  const field = leaseContractSchemaMap.get(fieldName);
