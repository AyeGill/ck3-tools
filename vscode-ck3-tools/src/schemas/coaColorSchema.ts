...
export const coaColorSchema: FieldSchema[] = [
  // Basic Properties
  {
    name: 'name',
    type: 'string',
...
export const coaColorSchemaMap = new Map<string, FieldSchema>(
  coaColorSchema.map((field) => [field.name, field])
);

export function getCoaColorFieldNames(): string[] {
  return coaColorSchema.map((field) => field.name);
}

export function getCoaColorFieldDocumentation(fieldName: string): string | undefined {
  const field = coaColorSchemaMap.get(fieldName);
