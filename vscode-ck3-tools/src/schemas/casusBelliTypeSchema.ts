...
export const casusBelliTypeSchema: FieldSchema[] = [
  // Basic Properties
  {
    name: 'name',
    type: 'string',
...
export const casusBelliTypeSchemaMap = new Map<string, FieldSchema>(
  casusBelliTypeSchema.map((field) => [field.name, field])
);

export function getCasusBelliTypeFieldNames(): string[] {
  return casusBelliTypeSchema.map((field) => field.name);
}

export function getCasusBelliTypeFieldDocumentation(fieldName: string): string | undefined {
  const field = casusBelliTypeSchemaMap.get(fieldName);
