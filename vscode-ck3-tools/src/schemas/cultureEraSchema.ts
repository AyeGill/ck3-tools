...
export const cultureEraSchema: FieldSchema[] = [
  // Basic Properties
  {
    name: 'year',
    type: 'integer',
...
export const cultureEraSchemaMap = new Map<string, FieldSchema>(
  cultureEraSchema.map((field) => [field.name, field])
);

export function getCultureEraFieldNames(): string[] {
  return cultureEraSchema.map((field) => field.name);
}

export function getCultureEraFieldDocumentation(fieldName: string): string | undefined {
  const field = cultureEraSchemaMap.get(fieldName);
