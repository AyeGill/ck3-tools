...
export const vassalStanceSchema: FieldSchema[] = [
  // Basic Properties
  {
    name: 'stance_type',
    type: 'enum',
...
export const vassalStanceSchemaMap = new Map<string, FieldSchema>(
  vassalStanceSchema.map((field) => [field.name, field])
);

export function getVassalStanceFieldNames(): string[] {
  return vassalStanceSchema.map((field) => field.name);
}

export function getVassalStanceFieldDocumentation(fieldName: string): string | undefined {
  const field = vassalStanceSchemaMap.get(fieldName);
