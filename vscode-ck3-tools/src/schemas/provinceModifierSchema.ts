...
export const provinceModifierSchema: FieldSchema[] = [
  // Basic Properties
  {
    name: 'name',
    type: 'string',
...
export const provinceModifierSchemaMap = new Map<string, FieldSchema>(
  provinceModifierSchema.map((field) => [field.name, field])
);

export function getProvinceModifierFieldNames(): string[] {
  return provinceModifierSchema.map((field) => field.name);
}

export function getProvinceModifierFieldDocumentation(fieldName: string): string | undefined {
  const field = provinceModifierSchemaMap.get(fieldName);
