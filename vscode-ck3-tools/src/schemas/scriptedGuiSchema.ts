...
export const scriptedGuiSchema: FieldSchema[] = [
  // Scope
  {
    name: 'scope',
    type: 'enum',
...
export const scriptedGuiSchemaMap = new Map<string, FieldSchema>(
  scriptedGuiSchema.map((field) => [field.name, field])
);

export function getScriptedGuiFieldNames(): string[] {
  return scriptedGuiSchema.map((field) => field.name);
}

export function getScriptedGuiFieldDocumentation(fieldName: string): string | undefined {
  const field = scriptedGuiSchemaMap.get(fieldName);
