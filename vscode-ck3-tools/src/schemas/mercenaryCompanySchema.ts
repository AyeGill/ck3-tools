...
export const mercenaryCompanySchema: FieldSchema[] = [
  // Basic Properties
  {
    name: 'name',
    type: 'string',
...
export const mercenaryCompanySchemaMap = new Map<string, FieldSchema>(
  mercenaryCompanySchema.map((field) => [field.name, field])
);

export function getMercenaryCompanyFieldNames(): string[] {
  return mercenaryCompanySchema.map((field) => field.name);
}

export function getMercenaryCompanyFieldDocumentation(fieldName: string): string | undefined {
  const field = mercenaryCompanySchemaMap.get(fieldName);
