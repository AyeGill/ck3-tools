...
export const vassalPowerSchema: FieldSchema[] = [
  // Basic Properties
  {
    name: 'name',
    type: 'string',
...
export const vassalPowerSchemaMap = new Map<string, FieldSchema>(
  vassalPowerSchema.map((field) => [field.name, field])
);

export function getVassalPowerFieldNames(): string[] {
  return vassalPowerSchema.map((field) => field.name);
}

export function getVassalPowerFieldDocumentation(fieldName: string): string | undefined {
  const field = vassalPowerSchemaMap.get(fieldName);
