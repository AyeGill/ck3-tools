...
export const domicileSchema: FieldSchema[] = [
  // Basic Properties
  {
    name: 'type',
    type: 'enum',
    description: 'Type of domicile.',
    values: [...DOMICILE_TYPES],
    example: 'type = manor',
  },
  {
    name: 'icon',
...
export const domicileSchemaMap = new Map<string, FieldSchema>(
  domicileSchema.map((field) => [field.name, field])
);

export function getDomicileFieldNames(): string[] {
  return domicileSchema.map((field) => field.name);
}

export function getDomicileFieldDocumentation(fieldName: string): string | undefined {
  const field = domicileSchemaMap.get(fieldName);
