...
export const administrativeDivisionSchema: FieldSchema[] = [
  // Basic Properties
  {
    name: 'name',
    type: 'string',
...
export const administrativeDivisionSchemaMap = new Map<string, FieldSchema>(
  administrativeDivisionSchema.map((field) => [field.name, field])
);

export function getAdministrativeDivisionFieldNames(): string[] {
  return administrativeDivisionSchema.map((field) => field.name);
}

export function getAdministrativeDivisionFieldDocumentation(fieldName: string): string | undefined {
  const field = administrativeDivisionSchemaMap.get(fieldName);
