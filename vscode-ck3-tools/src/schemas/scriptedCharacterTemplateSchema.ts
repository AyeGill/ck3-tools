...
export const scriptedCharacterTemplateSchema: FieldSchema[] = [
  // Basic Properties
  {
    name: 'name',
    type: 'string',
...
export const scriptedCharacterTemplateSchemaMap = new Map<string, FieldSchema>(
  scriptedCharacterTemplateSchema.map((field) => [field.name, field])
);

export function getScriptedCharacterTemplateFieldNames(): string[] {
  return scriptedCharacterTemplateSchema.map((field) => field.name);
}

export function getScriptedCharacterTemplateFieldDocumentation(fieldName: string): string | undefined {
  const field = scriptedCharacterTemplateSchemaMap.get(fieldName);
