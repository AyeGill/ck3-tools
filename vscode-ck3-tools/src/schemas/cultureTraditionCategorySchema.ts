...
export const cultureTraditionCategorySchemaMap = new Map<string, FieldSchema>(
  cultureTraditionCategorySchema.map((field) => [field.name, field])
);

export function getCultureTraditionCategoryFieldNames(): string[] {
  return cultureTraditionCategorySchema.map((field) => field.name);
}

export function getCultureTraditionCategoryFieldDocumentation(fieldName: string): string | undefined {
  const field = cultureTraditionCategorySchemaMap.get(fieldName);
