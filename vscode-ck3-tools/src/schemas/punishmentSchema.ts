...
export const punishmentSchemaMap = new Map<string, FieldSchema>(
  punishmentSchema.map((field) => [field.name, field])
);

export function getPunishmentFieldNames(): string[] {
  return punishmentSchema.map((field) => field.name);
}

export function getPunishmentFieldDocumentation(fieldName: string): string | undefined {
  const field = punishmentSchemaMap.get(fieldName);
