 ...
 */
function extractBlockContent(content: string, startIndex: number): { content: string; endIndex: number } | null {
  let depth = 0;
  let blockStart = -1;
 ...
 */
function findField(content: string, fieldName: string): {
  isBlock: boolean;
  value?: string;
  blockContent?: string;
} | null {
 ...
 */
function extractLocKeysFromBlock(blockContent: string): string[] {
  const keys: string[] = [];
 ...
 */
function countTrackLevels(traitContent: string): number {
 ...
 */
function extractTraitLocalization(traitName: string, traitContent: string): { key: string; defaultText: string }[] {
  const keys: { key: string; defaultText: string }[] = [];
 ...
 */
function extractLocalizationKeys(content: string, fileType: string): { key: string; defaultText: string }[] {
  const keys: { key: string; defaultText: string }[] = [];
 ...
 */
function extractDecisionLocalization(decisionName: string, decisionContent: string): { key: string; defaultText: string }[] {
  const keys: { key: string; defaultText: string }[] = [];
  const foundKeys = new Set<string>();
 ...
 */
function extractEventLocalization(namespace: string, eventId: string, eventContent: string): { key: string; defaultText: string }[] {
  const keys: { key: string; defaultText: string }[] = [];
  const foundKeys = new Set<string>();
 ...
 */
function extractInteractionLocalization(interactionName: string, interactionContent: string): { key: string; defaultText: string }[] {
  const keys: { key: string; defaultText: string }[] = [];
  const foundKeys = new Set<string>();
 ...
 */
function extractSchemeLocalization(schemeName: string, schemeContent: string): { key: string; defaultText: string }[] {
  const keys: { key: string; defaultText: string }[] = [];
  const foundKeys = new Set<string>();
 ...
 */
function extractMenAtArmsLocalization(maaName: string, maaContent: string): { key: string; defaultText: string }[] {
  const keys: { key: string; defaultText: string }[] = [];
 ...
 */
function extractCasusBelliLocalization(cbName: string, cbContent: string): { key: string; defaultText: string }[] {
  const keys: { key: string; defaultText: string }[] = [];
 ...
 */
function extractCultureLocalization(cultureName: string, cultureContent: string): { key: string; defaultText: string }[] {
  const keys: { key: string; defaultText: string }[] = [];
 ...
 */
function extractTraditionLocalization(traditionName: string, traditionContent: string): { key: string; defaultText: string }[] {
  const keys: { key: string; defaultText: string }[] = [];
 ...
 */
function extractReligionLocalization(religionName: string, religionContent: string): { key: string; defaultText: string }[] {
  const keys: { key: string; defaultText: string }[] = [];
  const foundKeys = new Set<string>();
 ...
 */
function extractBuildingLocalization(buildingName: string, buildingContent: string): { key: string; defaultText: string }[] {
  const keys: { key: string; defaultText: string }[] = [];
  const foundKeys = new Set<string>();
...
function getFileType(filePath: string): string | null {
  if (filePath.includes('/common/traits/') || filePath.includes('\\common\\traits\\')) return 'trait';
  if (filePath.includes('/common/buildings/') || filePath.includes('\\common\\buildings\\')) return 'building';
  if (filePath.includes('/common/decisions/') || filePath.includes('\\common\\decisions\\')) return 'decision';
  if (filePath.includes('/events/') || filePath.includes('\\events\\')) return 'event';
  if (filePath.includes('/common/character_interactions/') || filePath.includes('\\common\\character_interactions\\')) return 'interaction';
  if (filePath.includes('/common/schemes/') || filePath.includes('\\common\\schemes\\')) return 'scheme';
  if (filePath.includes('/common/on_action/') || filePath.includes('\\common\\on_action\\')) return 'on_action';
  if (filePath.includes('/common/men_at_arms_types/') || filePath.includes('\\common\\men_at_arms_types\\')) return 'men_at_arms';
  if (filePath.includes('/common/casus_belli_types/') || filePath.includes('\\common\\casus_belli_types\\')) return 'casus_belli';
  if (filePath.includes('/common/culture/cultures/') || filePath.includes('\\common\\culture\\cultures\\')) return 'culture';
  if (filePath.includes('/common/culture/traditions/') || filePath.includes('\\common\\culture\\traditions\\')) return 'tradition';
  if (filePath.includes('/common/religion/religions/') || filePath.includes('\\common\\religion\\religions\\')) return 'religion';
  return null;
}
 ...
 */
function findLocalizationDir(filePath: string): string | null {
...
export function registerGenerateLocalizationCommand(
  ...
  const disposable = vscode.commands.registerCommand('ck3-tools.generateLocalization', async () => {
    ...
    try {
      ...
      vscode.window.showInformationMessage(
        `Added ${newKeys.length} new localization keys to ${locFileName} (${existingKeys.size} keys already existed)`
      );
