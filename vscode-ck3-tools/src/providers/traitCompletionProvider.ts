...
import {
  ...
  SEX_VALUES,
  getSchemaForContext,
  getSchemaMapForContext,
} from '../schemas/traitSchema';

/**
 * Provides autocomplete suggestions for CK3 trait files
 */
export class TraitCompletionProvider implements vscode.CompletionItemProvider {
  ...
  private getFieldCompletions(linePrefix: string, blockPath: string[]): vscode.CompletionItem[] {
  ...
  private getValueCompletions(
    ...
    const schemaMap = getSchemaMapForContext(context.blockPath || []);
    const field = schemaMap.get(fieldName);

    // If field not found in context schema, try the trait schema as fallback
    const effectiveField = field || traitSchemaMap.get(fieldName);
    ...
    switch (effectiveField.type) {
      ...
      case 'boolean':
        items.push(
          this.createSimpleCompletion('yes', 'Enable this option'),
          this.createSimpleCompletion('no', 'Disable this option')
        );
        break;
  ...
  private getIntegerSuggestions(fieldName: string, field: FieldSchema): [string, string][] {
    ...
    // Ruler designer cost
      ...
      suggestions.push(
        ['-50', 'Bad trait reward'],
        ['-25', 'Minor bad trait reward'],
        ['0', 'Neutral'],
        ['25', 'Minor good trait cost'],
        ['50', 'Good trait cost'],
        ['100', 'Very good trait cost']
      );
    }
    // AI value
    ...
    // Default
      ...
      if (field.min !== undefined && field.max !== undefined) {
        ...
        suggestions.push(
          [String(field.min), 'Minimum'],
          [String(mid), 'Middle'],
          [String(field.max), 'Maximum']
        );
      } else {
  ...
  private getFloatSuggestions(fieldName: string): [string, string][] {
  ...
  private getTopLevelCompletions(): vscode.CompletionItem[] {
    ...
    item.documentation = new vscode.MarkdownString(
      'Creates a new trait definition. Replace `trait_name` with your trait\'s unique identifier.'
    );
...
interface ParseContext {
  insideTraitBlock: boolean;
  afterEquals: boolean;
  fieldName?: string;
  partialValue?: string;
  braceDepth: number;
  /** The path of block names we're nested inside (excluding the trait name itself) */
  blockPath?: string[];
}
